...
class Coreapp_auth extends CI_Driver {
	...
	public function login() {
	    ...
	    if($this->logged_in){
            ...
            if($this->_insert_key( $this->key, ['user_id' => $this->user->id, 'level' => 10], $this->key_validity )){
                $return['status'] = TRUE;
	...
	public function register(){
        ...
        $email              = $this->CI->input->post('email');
		$password           = $this->CI->input->post('password');
		$confirm_password   = $this->CI->input->post('confirmPassword');
        
        $user_id = $this->CI->ion_auth->register($email, $password, $email);
        ...
        if($user_id){
            ...
            $return['status'] = FALSE;
	        $return['msg'] = $this->messages['register_failure'];
        }
    ...
    //         return FALSE;
    //     }
        if($this->CI->ion_auth->login($email, $password, FALSE)){
